...
class Ebox_Cliqueretire_Shipment
{
    /**
     * Response Messages
     */
    ...
    /**
     * Handle a Cliqueretire Shipment Request
     *
     * @return void
     */
    ...
    public function updateOrder($order, $requestData)
    {
        ...
        if (get_post_meta($orderId, '_ebox_cliqueretire_shipped_items', true)) {
            $totalItemsShipped = get_post_meta($orderId, '_ebox_cliqueretire_shipped_items', true);
        }
        // If no items have been shipped previously set count to 0
        else {
            add_post_meta($orderId, '_ebox_cliqueretire_shipped_items', 0, true);
            $totalItemsShipped = 0;
        }

        // Add order comment for when items are shipped
        $orderComment = 'The following items have been marked as Shipped in Cliqueretire..<br>';
        ...
        if ($totalItemsShipped >= $totalItemsShippable) {
            $order->update_status('completed', 'Order has been shipped with Cliqueretire');
        ...
        update_post_meta($orderId, '_ebox_cliqueretire_shipped_items', $totalItemsShipped);
    }
    ...
    public function updateShipmentMetadata($orderId, $requestData, $totalItemsShipped)
    {
        ...
        $shipments = array();
        $existingShipment = get_post_meta($orderId, '_ebox_cliqueretire_shipment', false);
        ...
        update_post_meta($orderId, '_ebox_cliqueretire_shipment', $shipments);
    }
    ...
    public function checkApiKey()
    {
        ...
        $apiKey = get_option('wc_settings_cliqueretire_api_key');

        // Grab the posted cliqueretire API key
        $requestApiKey = $wp->query_vars['cliqueretire_api_key'];
    ...
    public function checkRequest($requestData)
    {
        ...
        $this->log->add(
            ...
            array(
                'url' => get_site_url() . '/cliqueretire/shipment_create?cliqueretire_api_key=' . $requestApiKey,
