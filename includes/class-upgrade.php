...
class Ebox_Cliqueretire_Upgrade
{
    const OPTIONS_PREFIX = 'wc_settings_cliqueretire_';

    public function run()
    {
        $dbVersion = get_option('wc_cliqueretire_version', 0);

        // If an upgrade is not required, stop here
        if (version_compare(EBOX_CLIQUERETIRE_VERSION, $dbVersion, '<=')) {
            return;
        }
    ...
    protected function upgrade_1_3_0($dbVersion)
    {
        ...
        // Migrate the core module settings to the new "Cliqueretire Tab"
        $oldOptions = get_option('woocommerce_ebox_cliqueretire_settings');
        ...
        update_option('woocommerce_ebox_cliqueretire_legacy_settings', $oldOptions);

        // Update version
        update_option('wc_cliqueretire_version', '1.3.0');
    }
    ...
    protected function upgrade_1_4_0($dbVersion)
    {
        ...
        // Update version
        update_option('wc_cliqueretire_version', '1.4.0');
    }
    ...
    protected function upgrade_1_5_5($dbVersion)
    {
        ...
        // Update version
        update_option('wc_cliqueretire_version', '1.5.5');
    }

    protected function upgrade_complete($dbVersion)
    {
        // Update DB version to latest code release version
        update_option('wc_cliqueretire_version', EBOX_CLIQUERETIRE_VERSION);
    }
}
