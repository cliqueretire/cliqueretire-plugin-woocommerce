...
class Ebox_Cliqueretire_Data_Mapper_Order_V26 extends Ebox_Cliqueretire_Object
{
    protected $helper;
    protected $order;
    ...
    {
        if ($this->helper->isCliqueretireLiveQuote($this->order)) {
            // If a cliqueretire live quote is available, we'll set a courier allocation
    ...
    {
        if ($this->helper->isCliqueretireLiveQuote($this->order)) {
            $courierAllocation = $this->helper->getCliqueretireLiveQuoteDetail($this->order, 'courier_allocation');
    ...
    {
        // Only provide a delivery date if the order has a cliqueretire live quote
        if (!$this->helper->isCliqueretireLiveQuote($this->order)) {
            return $this;
        }

        $deliveryDate = $this->helper->getCliqueretireLiveQuoteDetail($this->order, 'delivery_date');
    ...
    public function mapDeliveryWindow()
    {
        ...
        $deliveryWindow = $this->helper->getCliqueretireLiveQuoteDetail($this->order, 'delivery_window');
    ...
    public function mapParcelAttributes()
    {
        ...
        $orderItemDataMapper = new Ebox_Cliqueretire_Data_Mapper_Order_Item_V26();
